...
$(function () {
    ...
    $("input[name='fisforbidden']").change(function () {
        var checked = $(this).is(":checked");
        var $status_label = $("#fisforbidden");
   ...
   //     getDept();
        ...
        if (flag) {
            if (name === "save") {
                $.post(ctx + "userEntity/add", $userAddForm.serialize(), function (r) {
                    if (r.code === 0) {
                        closeModal();
                        $MB.n_success(r.msg);
            ...
            if (name === "update") {
                $.post(ctx + "userEntity/update", $userAddForm.serialize(), function (r) {
                    if (r.code === 0) {
                        closeModal();
                        $MB.n_success(r.msg);
                        $MB.refreshTable("userTable");
...
function closeModal() {
    ...
    $userAddForm.find("input[name='fisforbidden']").prop("checked", true);
    $("#user-add-modal-title").html('新增用户');
    $("#fisforbidden").html('可用');
    ...
    validator = $userAddForm.validate({
        rules: {
            username: {
                ...
                maxlength: 10,
                remote: {
                    url: "userEntity/checkUserName",
                    type: "get",
                    dataType: "json",
                    data: {
                        fname: function () {
                            return $("input[name='fname']").val().trim();
                        },
